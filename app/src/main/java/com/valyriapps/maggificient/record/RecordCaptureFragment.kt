...
import com.valyriapps.maggificient.MainActivity.Companion.VIDEO_PERMISSIONS
import com.valyriapps.maggificient.R
import com.valyriapps.maggificient.calculateFingerSpacing
import com.valyriapps.maggificient.common.FileType
import com.valyriapps.maggificient.common.GestureListener
import com.valyriapps.maggificient.common.PermissionsUtils
import com.valyriapps.maggificient.common.SwipeGesture
import com.valyriapps.maggificient.performHapticContextClick
import com.valyriapps.maggificient.saveFileExternally
...
class RecordCaptureFragment : Fragment(), SwipeGesture {
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        override fun onOpened(cameraDevice: CameraDevice) {
            ...
            maggificientCameraDevice = cameraDevice

            startPreview()
            this@RecordCaptureFragment.activity?.configureTransform(
                videoSize,
                textureView.width,
                textureView.height
            )
    ...
    private fun launchPreviewFragment(videoUrl: String, isGalleryVideo: Boolean = false) {
        ...
        this.activity?.apply {
            supportFragmentManager.beginTransaction()
                .replace(
                    R.id.routingContainer,
                    previewFragment,
                    previewFragment.javaClass.simpleName
                )
    ...
    private fun pinchToZoom(event: MotionEvent) {
        ...
        if (fingerSpacing != 0F) {
            ...
            val croppedWidth = sensorRect.width() - (sensorRect.width() * ratio).roundToInt()
            val croppedHeight = sensorRect.height() - (sensorRect.height() * ratio).roundToInt()
    ...
    private fun closeCamera() {
        try {
            ...
            closePreviewSession()
            maggificientCameraDevice?.close()
            maggificientCameraDevice = null
    ...
    private fun startPreview() {
        try {
            ...
            previewRequestBuilder =
                maggificientCameraDevice!!.createCaptureRequest(CameraDevice.TEMPLATE_RECORD)
            previewRequestBuilder.addTarget(previewSurface)

            maggificientCameraDevice?.createCaptureSession(
    ...
    private fun startRecordingVideo() {
        ...
        try {
            ...
            val cameraDevice = maggificientCameraDevice ?: return
            previewRequestBuilder =
